...
namespace question2 {
    class a4q2forloop {
        static void Main(string[] args) {
            ...
            for (double i = startingFarenheit; i <= endingFarenheit; i = i + increment) {
                Console.WriteLine("{0,-12} {1,-12}", $"{i.ToString("F", CultureInfo.InvariantCulture)}", $"{((i - 32)/ 1.8).ToString("F", CultureInfo.InvariantCulture)}");
            }
        }
    }
}